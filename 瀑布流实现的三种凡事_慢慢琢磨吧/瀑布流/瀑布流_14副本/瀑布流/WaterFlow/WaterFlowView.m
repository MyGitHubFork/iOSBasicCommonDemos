...
#warning 当self.contentOffset.y > self.contentSize.height的时候，开始抓取新的数据
    if (self.contentOffset.y + self.bounds.size.height > self.contentSize.height) {
        //刷新网络数据，并刷新数据显示。视图控制器负责刷新数据。
        //加载数据完成后，通知视图刷新显示。
        [self.delegate waterFlowViewRefreshData:self];
    }
    NSLog(@"layout subviews %lu %f", (unsigned long)self.subviews.count, self.contentOffset.y);
}
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    for (NSIndexPath *indexPath in keyArray) {
        WaterFlowCellView *cell = self.screenCellsDict[indexPath];
#warning 如果一个点在cell的范围之内
        //3依次判断手指点击位置是否在单元格内部。
        if(CGRectContainsPoint(cell.frame, location)){
            //4如果是，返回该单元格对应的indexpath
